...
| 4 | Icarus **네이티브 정책(LRU/LFU 등)과의 비교 부재** — PassiveAgentCache 미활용 | **P1** |
| 5 | AdaptSize 대체 baseline (best static size-threshold) 부재 | **P1** |
| 6 | 다중 seed 평가 + 신뢰구간 (현재 best-checkpoint 단일 수치) | **P1** |
| 7 | "훈련이 IcarusGym 밖(standalone sim)"인 점의 논문 서술 전략 미정 | **P1** |
| 8 | stale 주석/문서, trace wrap-around 등 코드 정리 | P2 |
...
- 논문에는 "재현에 필요했던 플랫폼 측 코드 몇 줄"을 같이 명시 (P1-7의 LOC 집계와 연결).
...
#### P1-4. Icarus 네이티브 정책과의 비교 (PassiveAgentCache 활용)
...
#### P1-5. AdaptSize 대체: best static size-threshold baseline (~30 lines)
...
#### P1-7. "훈련이 IcarusGym 밖"인 점의 논문 서술 전략 결정
...
3. **feature 구성의 논문 대비 편차 문서화**: `f/s`, `f·s`가 raw size가 아닌 **normalized log-size**
   기반으로 계산됨(`features.py:96-99`) — 원 논문 Table 1의 의미(작고 인기 있는 object 선호)와
   semantic이 다름. GitHub 참조 구현의 정규화 방식 확인 후(아래 §4) 정합시키거나 의도적 편차로
   README에 명시.
...
## 4. 미결: GitHub 참조 구현 대조 (백그라운드 조사)

`https://github.com/quovadim/RL-Cache` (구 WVadim/RL-Cache)에 대해 다음 항목의 사실 확인을
백그라운드 에이전트로 조사 중 (완료 시 이 섹션 업데이트 예정):
- NN 구조(층수/유닛/활성함수/dropout), optimizer/lr
- Table-1 feature의 실제 정규화 방식 (f/s를 raw size로 계산하는지), EMA α 값
- K/m/p/L/γ/q 기본값, BCE target이 marginal인지, 윈도우 슬라이드 방식
- LRU eviction과 admission의 상호작용, trace CSV 포맷, train/test split 처리

→ 확인 결과에 따라 P2-3(feature 편차)의 처리 방향 결정.
...
- **빠른 재개 순서 (회사에서):**
  ...
  3. `P1-5`: static size-threshold baseline을 `baselines.py`에 추가 + sweep에 편입. (1–2시간)
